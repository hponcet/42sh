...
/*   Updated: 2016/09/01 19:39:19 by MrRobot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int		ft_read_buff(int const fd, t_get **link, char **line)
{
	...
	if (oct == -1 || ((*link)->save == NULL && oct == 0))
		return (oct);
	while (oct > 0 && ft_strchr_index((*link)->buff, '\n') == -1)
	{
		if ((*link)->save == NULL)
...
int				ft_get_next_line(int const fd, char **line)
{
	...
	ft_strclr(link->buff, BUFF_SIZE + 1);
	if (link->save != NULL && (i = ft_strchr_index(link->save, '\n')) != -1)
	{
