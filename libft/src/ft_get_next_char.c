...
/*   Updated: 2016/11/28 10:38:41 by MrRobot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	int		ft_back_slash_in_buff(char **line, t_get *link, int test, char c)
{
	char	*tmp;
	int		i;
...
static	int		ft_read_buff(int const fd, t_get **link, char **line, char c)
{
	int		oct;
	char	*tmp;
	...
	if (oct == -1 || ((*link)->save == NULL && oct == 0))
		return (oct);
	while (oct > 0 && ft_strchr_index((*link)->buff, c) == -1)
	{
		if ((*link)->save == NULL)
	...
	if (oct == 0 && ((*link)->save) != NULL)
		return (ft_back_slash_in_buff(line, *link, 0, c));
	else
		return (ft_back_slash_in_buff(line, *link, 1, c));
}

int				ft_get_next_line(int const fd, char **line, char c)
{
	...
	ft_strclr(link->buff, BUFF_SIZE + 1);
	if (link->save != NULL && (i = ft_strchr_index(link->save, c)) != -1)
	{
		*line = ft_strsub(link->save, 0, i);
		...
		return (ft_read_buff(fd, &link, line, c));
	return (1);
}
