...
/*   Updated: 2016/07/10 16:41:13 by MrRobot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (tab[*i + 1] == NULL)
		return (ft_error(4, "env"));
	else if (ft_strchr_index(tab[*i + 1], '=') > -1)
		return (ft_error(6, tab[*i + 1]));
...
int			ft_env(char **tab, t_list *env)
{
	...
	while (tab[i] != NULL)
	{
		...
		else if ((j = ft_strchr_index(tab[i], '=')) > 0)
